...
for epoch in range(1,epoch+1):
    x0_train,x1_train,x2_train = tb.instance(x_train,y_train,num_train_instance)
    x0_test,x1_test,x2_test = tb.instance(x_test,y_test,num_test_instance)
    ...
    for i in range(int(num_train_instance/batch_size)):
        ...
        x2_batch = torch.from_numpy(x2_train[i*batch_size:(i+1)*batch_size]).cuda()         # pylint: disable=no-member # pytorch/issues/701

        optimizer.zero_grad()
        positive, anchor, negative = net(x0_batch, x1_batch, x2_batch)
        loss = tripletLoss(anchor,positive,negative,size_average=True)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i in range(int(num_train_instance/batch_size)):
            ...
            positive, anchor, negative = net(x0_batch, x1_batch, x2_batch)
            loss = tripletLoss(anchor,positive,negative,size_average=False)
            loss2 += float(loss)
            bar2.next()
        bar2.finish()
        ...
        for i in range(int(num_test_instance/batch_size)):
            x0_test_batch = torch.from_numpy(x0_test[i*batch_size:(i+1)*batch_size]).cuda() # pylint: disable=no-member # pytorch/issues/701
            x1_test_batch = torch.from_numpy(x1_test[i*batch_size:(i+1)*batch_size]).cuda() # pylint: disable=no-member # pytorch/issues/701
            x2_test_batch = torch.from_numpy(x2_test[i*batch_size:(i+1)*batch_size]).cuda() # pylint: disable=no-member # pytorch/issues/701

            positive, anchor, negative = net(x0_test_batch, x1_test_batch, x2_test_batch)
            loss = tripletLoss(anchor,positive,negative,size_average=False)
            loss3 += float(loss)
            bar3.next()
        bar3.finish()
    ...
    encoded_test, encoded_train = tb.encode(x_train,x_test,128,'last_param.pt', to_csv=False, use_cpu=False)
...
encoded_test, encoded_train = tb.encode(x_train, x_test, 128, 'best_param.pt')
